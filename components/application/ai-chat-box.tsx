...
export function AIChatBox() {
  ...
  const [results, setResults] = useState<EntertainmentContent[]>([]);
  ...
  return (
    <section className="batch4-chat">
      ...
      {results.length > 0 && <div className="batch4-chat-results">{results.map((item) => <Link href={`/search?q=${encodeURIComponent(item.title)}`} key={item.id}><img src={item.posterUrl} alt="" /><span><b>{item.title}</b><small>{item.reason}</small></span></Link>)}</div>}
